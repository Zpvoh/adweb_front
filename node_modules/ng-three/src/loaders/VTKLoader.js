...
Object.assign( VTKLoader.prototype, {
	...
	load: function ( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function( text ) {
			...
			var resp = scope.parse( text );
			
			resp.geometry.center();
			resp.geometry.computeVertexNormals();

			onLoad( resp.geometry );
	...
	parse: function ( data ) {
		...
		if ( meta[ 0 ].indexOf( 'xml' ) !== - 1 ) {
			...
			return { type: 'xml', geometry: this.parseXML( this.getStringFile( data ) ) };

		} else if ( meta[ 2 ].includes( 'ASCII' ) ) {

			return { type: 'ASCII', geometry: this.parseASCII( this.getStringFile( data ) ) };

		} else {

			return { type: 'binary', geometry: this.parseBinary( data ) };
	...
	findString: function( buffer, start ) {
	...
	Int32Concat: function(first, second) {
	    var firstLength = first.length,
	...
	Base64toByteArray: function( b64 ) {
	...
	parseDataArray: function( ele, compressed ) {
		var numBytes = 0;
		...
		if ( ele.attributes.format == 'binary' && compressed ) {
				...
				byteData = this.Base64toByteArray( rawData );
				...
				// There are blocks before c-size-i, so we skip 3*numBytes
			...
			if ( ele.attributes.format == 'binary' && ! compressed ) {
				...
				var content = this.Base64toByteArray( ele[ '#text' ] );
				...
				if ( section.DataArray ) {
					...
					while ( dataArrayIndex < numberOfDataArrays ) {
						...
						if ( ('#text' in arr[ dataArrayIndex ]) && (arr[ dataArrayIndex ][ '#text' ].length > 0) ) {
							arr[ dataArrayIndex ].text = this.parseDataArray( arr[ dataArrayIndex ], compressed );
						}
							dataArrayIndex ++;
